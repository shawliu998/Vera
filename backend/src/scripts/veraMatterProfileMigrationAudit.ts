...
function auditV15ClassificationUpgradeAndRestart() {
  ...
  try {
    assert.equal(classificationRequiredRestart.migration?.currentVersion, 24);
  ...
  try {
    assert.equal(classifiedRestart.migration?.currentVersion, 24);
    assert.deepEqual(
      object(
        classifiedRestart
          .prepare(
  ...
  try {
    assert.equal(currentBinaryRecovery.migration?.currentVersion, 24);
    ...
    JSON.stringify(
      {
        ...
        current_version: 24,
        checks: [
          "clean SQLite v24 install with additive inference-policy, Legal Provider, Assistant action, Draft metadata, templates, research, Tabular handoff, and contract-review slices",
