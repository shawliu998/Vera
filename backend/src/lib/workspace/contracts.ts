...
export const OptionalDescriptionSchema = z.string().trim().min(1).max(2_000).nullable();
export const CursorSchema = z.string().min(1).max(512);

export const ProjectStatusSchema = z.enum(["active", "archived", "deleted"]);
export const DocumentStatusSchema = z.enum(["pending", "processing", "ready", "failed", "unsupported", "ocr_required"]);
export const DocumentVersionSourceSchema = z.enum(["upload", "user_upload", "assistant_edit"]);
export const DocumentEditStatusSchema = z.enum(["pending", "accepted", "rejected"]);
export const ChatScopeSchema = z.enum(["global", "project"]);
export const ChatStatusSchema = z.enum(["active", "archived"]);
export const MessageRoleSchema = z.enum(["system", "user", "assistant", "tool"]);
export const MessageStatusSchema = z.enum(["pending", "streaming", "complete", "failed", "cancelled", "interrupted"]);
export const ModelProviderSchema = z.enum(["openai", "deepseek", "anthropic", "gemini", "openai_compatible"]);
export const CredentialStatusSchema = z.enum(["not_configured", "configured", "unavailable"]);
export const WorkflowTypeSchema = z.enum(["assistant", "tabular"]);
export const WorkflowStatusSchema = z.enum(["active", "archived"]);
export const RunStatusSchema = z.enum(["queued", "waiting", "running", "complete", "failed", "cancelled", "interrupted"]);
export const StepRunStatusSchema = z.enum(["queued", "waiting", "running", "complete", "failed", "cancelled", "interrupted", "skipped"]);
export const TabularOutputTypeSchema = z.enum(["text", "boolean", "enum", "number"]);
export const TabularReviewStatusSchema = z.enum(["draft", "ready", "running", "complete", "failed", "cancelled", "archived"]);
export const TabularCellStatusSchema = z.enum(["empty", "queued", "running", "complete", "failed", "cancelled"]);
export const JobTypeSchema = z.enum(["document_parse", "assistant_generate", "workflow_run", "tabular_cell"]);
export const JobStatusSchema = z.enum(["queued", "running", "complete", "failed", "cancelled", "interrupted"]);
export const LegacyImportStatusSchema = z.enum(["pending", "running", "complete", "failed", "skipped"]);
...
export const DocumentChunkSchema = z.object({
  ...
  if (value.endOffset < value.startOffset) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["endOffset"], message: "endOffset must not precede startOffset" });
  }
  if (value.pageStart !== null && value.pageEnd !== null && value.pageEnd < value.pageStart) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["pageEnd"], message: "pageEnd must not precede pageStart" });
  }
});
...
export const ChatSchema = z.object({
  ...
  message: "project chats require a projectId and global chats must not include one",
  path: ["projectId"],
});
...
const BaseUrlSchema = z.string().url().max(500).refine((value) => {
...
export const PromptWorkflowStepSchema = z.object({
  kind: z.literal("prompt"),
  title: z.string().min(1).max(160),
  prompt: z.string().min(1).max(20_000),
}).strict();

export const DocumentContextWorkflowStepSchema = z.object({
...
const TabularColumnWorkflowStepObjectSchema = z.object({
...
export const WorkflowStepSchema = z.discriminatedUnion("kind", [
  ...
  if (value.outputType === "enum" && !value.enumValues?.length) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["enumValues"], message: "enum columns require enumValues" });
  }
  if (value.outputType !== "enum" && value.enumValues) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["enumValues"], message: "enumValues are only valid for enum columns" });
  }
});
...
export const AssistantWorkflowSchema = WorkflowBaseSchema.extend({
  ...
  skillMarkdown: z.string().min(1).max(100_000),
});

export const TabularWorkflowSchema = WorkflowBaseSchema.extend({
  type: z.literal("tabular"),
  columns: z.array(WorkflowColumnSchema).min(1).max(100),
});

export const WorkflowSchema = z.discriminatedUnion("type", [AssistantWorkflowSchema, TabularWorkflowSchema]);

type SafeStructuredValue = string | number | boolean | null | SafeStructuredValue[] | { [key: string]: SafeStructuredValue };
const blockedStructuredKeys = /(?:secret|api[_-]?key|token|storage[_-]?path|absolute[_-]?path|file[_-]?path)/i;
export const SafeStructuredValueSchema: z.ZodType<SafeStructuredValue> = z.lazy(() => z.union([
  z.string().max(200_000),
  z.number().finite(),
  z.boolean(),
  z.null(),
  z.array(SafeStructuredValueSchema).max(10_000),
  z.record(SafeStructuredValueSchema).superRefine((value, context) => {
    for (const key of Object.keys(value)) {
      if (blockedStructuredKeys.test(key)) {
        context.addIssue({ code: z.ZodIssueCode.custom, path: [key], message: "unsafe structured key is not allowed" });
      }
    }
  }),
]));
...
export const StructuredErrorSchema = z.object({
  ...
  retryable: z.boolean(),
  details: z.record(z.union([z.string().max(2_000), z.number().finite(), z.boolean(), z.null()])).superRefine((value, context) => {
    for (const key of Object.keys(value)) {
      if (blockedStructuredKeys.test(key)) {
        context.addIssue({ code: z.ZodIssueCode.custom, path: [key], message: "unsafe error detail key is not allowed" });
      }
    }
  }).nullable(),
}).strict();
...
export const TabularReviewSchema = z.object({
  id: WorkspaceIdSchema,
  projectId: NullableWorkspaceIdSchema,
  workflowId: NullableWorkspaceIdSchema,
  title: z.string().min(1).max(240),
  status: TabularReviewStatusSchema,
  documentIds: z.array(WorkspaceIdSchema).min(1).max(1_000),
  modelProfileId: NullableWorkspaceIdSchema,
  createdAt: IsoDateTimeSchema,
  updatedAt: IsoDateTimeSchema,
}).strict();
...
export const TabularCellValueSchema = z.union([z.string().max(20_000), z.boolean(), z.number().finite(), z.null()]);
export const TabularCellSchema = z.object({
  id: WorkspaceIdSchema,
  reviewId: WorkspaceIdSchema,
  documentId: WorkspaceIdSchema,
  ...
  if (!valid) context.addIssue({ code: z.ZodIssueCode.custom, path: ["value"], message: "value does not match outputType" });
});

export const TabularReviewChatSchema = z.object({
  id: WorkspaceIdSchema,
  reviewId: WorkspaceIdSchema,
  title: z.string().min(1).max(240),
  createdAt: IsoDateTimeSchema,
...
export const TabularReviewChatMessageSchema = z.object({
  id: WorkspaceIdSchema,
...
export const JobSchema = z.object({
  ...
  resourceType: z.enum(["document", "chat", "workflow_run", "tabular_cell", "tabular_review", "project"]),
...
export const CreateProjectFolderRequestSchema = z.object({
  name: z.string().trim().min(1).max(160),
  parentFolderId: NullableWorkspaceIdSchema.optional(),
}).strict();

export const UpdateProjectFolderRequestSchema = z.object({
...
export const ResolveDocumentEditRequestSchema = z.object({
...
export const UpdateModelProfileRequestSchema = CreateModelProfileRequestSchema.partial().refine((value) => Object.keys(value).length > 0, "at least one update is required");

const WorkflowColumnInputSchema = z.object({
  key: z.string().regex(/^[a-z][a-z0-9_]{0,63}$/),
  title: z.string().trim().min(1).max(160),
  outputType: TabularOutputTypeSchema,
  prompt: z.string().trim().min(1).max(20_000),
  enumValues: z.array(z.string().trim().min(1).max(160)).min(1).max(100).optional(),
}).strict().superRefine((value, context) => {
  if (value.outputType === "enum" && !value.enumValues?.length) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["enumValues"], message: "enum columns require enumValues" });
  }
...
export const CreateAssistantWorkflowRequestSchema = z.object({
  type: z.literal("assistant"),
  title: z.string().trim().min(1).max(200),
  description: OptionalDescriptionSchema.optional(),
  skillMarkdown: z.string().trim().min(1).max(100_000),
  steps: z.array(WorkflowStepSchema).max(100).default([]),
}).strict();

export const CreateTabularWorkflowRequestSchema = z.object({
  type: z.literal("tabular"),
  title: z.string().trim().min(1).max(200),
  description: OptionalDescriptionSchema.optional(),
  columns: z.array(WorkflowColumnInputSchema).min(1).max(100),
  steps: z.array(WorkflowStepSchema).max(100).default([]),
}).strict();

export const CreateWorkflowRequestSchema = z.discriminatedUnion("type", [CreateAssistantWorkflowRequestSchema, CreateTabularWorkflowRequestSchema]);

export const UpdateWorkflowRequestSchema = z.object({
  title: z.string().trim().min(1).max(200).optional(),
  description: OptionalDescriptionSchema.optional(),
  status: WorkflowStatusSchema.optional(),
  skillMarkdown: z.string().trim().min(1).max(100_000).optional(),
  steps: z.array(WorkflowStepSchema).max(100).optional(),
  columns: z.array(WorkflowColumnInputSchema).min(1).max(100).optional(),
}).strict().refine((value) => Object.keys(value).length > 0, "at least one update is required");
...
export const CreateTabularReviewRequestSchema = z.object({
  projectId: NullableWorkspaceIdSchema.optional(),
...
export const UpdateTabularReviewRequestSchema = z.object({
  title: z.string().trim().min(1).max(240).optional(),
  status: z.enum(["draft", "ready", "archived", "cancelled"]).optional(),
  modelProfileId: NullableWorkspaceIdSchema.optional(),
}).strict().refine((value) => Object.keys(value).length > 0, "at least one update is required");

export const GenerateTabularCellRequestSchema = z.object({
  modelProfileId: NullableWorkspaceIdSchema.optional(),
}).strict();

export const CreateTabularReviewChatRequestSchema = z.object({
  title: z.string().trim().min(1).max(240).optional(),
}).strict();

export const CreateTabularReviewChatMessageRequestSchema = z.object({
  content: z.string().trim().min(1).max(100_000),
  modelProfileId: NullableWorkspaceIdSchema.optional(),
}).strict();
...
export const ApiErrorSchema = z.object({
  error: z.object({
    ...
    details: z.array(z.object({ path: z.string().max(500), message: z.string().min(1).max(2_000) }).strict()).max(100).optional(),
  }).strict(),
}).strict();
...
export type CreateTabularReviewRequest = z.infer<typeof CreateTabularReviewRequestSchema>;
