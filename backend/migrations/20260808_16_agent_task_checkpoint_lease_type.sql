-- Kernel v1 Batch 16: keep checkpoint ownership on the same UUID lease type
-- as agent_tasks.execution_lease_owner. The prior text argument caused every
-- otherwise-valid publication checkpoint to fail at PostgreSQL operator lookup.
...
revoke execute on function public.commit_agent_task_checkpoint_v1(
  uuid, text, uuid, text, uuid, integer, jsonb, uuid[]
...
comment on function public.commit_agent_task_checkpoint_v1(
  uuid, text, uuid, text, uuid, integer, jsonb, uuid[]
) is 'Atomically checkpoints bounded server-owned Step progress under the exact active UUID execution lease.';
