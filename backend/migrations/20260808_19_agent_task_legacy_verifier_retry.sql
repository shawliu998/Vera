-- Forward-compatible Verifier retry: current Tasks require an exact structured
-- result. A historical Task created before structured records existed may
-- restart only the Verifier from its exact legacy review-gap receipt. No repair
-- is selected from legacy summary text; only the fresh structured result can
-- authorize a later bounded mutation.
    ...
    or exists (
      ...
      where 1 <> (
        ...
        where jsonb_typeof(identity.value) = 'object'
          and identity.value ->> 'kind' in (
            'agent_verified_draft_artifact_v1',
            'agent_verified_tabular_artifact_v1'
          )
          and coalesce(
...
comment on function public.start_agent_task_verifier_retry_v1(
  ...
  'Atomically restarts only the final Verifier from an exact current structured result or a fail-closed legacy review-gap receipt; it never selects or mutates an Artifact from legacy summary text.';
