-- Kernel v1 compatibility: preserve completed fixed Tabular cells when a Task\n-- resumes on a new Step attempt. The layout, Matter, owner and lease remain\n-- server-enforced; only pending or already-done fixed coordinates are valid.
    ...
    and not exists (
      ...
      where not exists (
        ...
        where cell.review_id = review.id
          ...
          and cell.status in ('pending', 'done')
      )
    )
