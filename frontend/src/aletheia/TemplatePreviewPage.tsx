        ...
        <section className="mt-4 grid gap-4 lg:grid-cols-[320px_1fr]">
          <aside className="space-y-4">
            <div className="rounded-lg border border-[#e5e7eb] bg-white p-4">
              ...
              <Badge
                variant="outline"
                className={`mt-3 rounded-md ${riskClass(matter.riskLevel ?? "medium")}`}
              >
          ...
          <div className="space-y-4">
                    ...
                    {isCompliance
                      ? complianceObligations.map((item) => (
                          <tr key={item.id}>
                            ...
                            <td className="px-3 py-3 align-top text-[#6b7280]">
                              {item.owner}: {item.remediation}
                            </td>
                            <td className="px-3 py-3 align-top">
                              <Badge
                                variant="outline"
                                className={`rounded-md ${riskClass(item.riskLevel)}`}
                              >
                                {item.riskLevel}
                              </Badge>
                            </td>
                      ...
                      : dealRedFlags.map((item) => (
                          <tr key={item.id}>
                            ...
                            <td className="px-3 py-3 align-top">
                              <Badge
                                variant="outline"
                                className={`rounded-md ${riskClass(item.severity)}`}
                              >
                                {item.severity}
                              </Badge>
                            </td>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-lg border border-[#e5e7eb] bg-white p-4">
                <h2 className="font-semibold">Template Work Products</h2>
                <div className="mt-3 space-y-2">
                  {template.outputs.map((output) => (
                    <div
                      key={output}
                      className="flex items-center gap-2 rounded-md border border-[#e5e7eb] p-3 text-sm"
                    >
              ...
              <div className="rounded-lg border border-[#e5e7eb] bg-white p-4">
                <h2 className="font-semibold">Local Workflow Status</h2>
                <p className="mt-3 text-sm leading-6 text-[#6b7280]">
                  This template now uses the same local Matter Workspace,
                  document upload, evidence mapping, human review, and audit
                  primitives as the Legal Matter Review flow. Create a local
                  matter from this template to generate source-linked work
                  products from uploaded documents.
                </p>
                <Button
                  asChild
                  className="mt-4 bg-[#111827] text-white hover:bg-[#1f2937]"
                >
                  <Link href="/aletheia">Open Matter Queue</Link>
                </Button>
              </div>
            </div>
          </div>
