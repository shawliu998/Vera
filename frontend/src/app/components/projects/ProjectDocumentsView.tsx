...
export function ProjectDocumentsView({ projectId }: Props) {
    ...
    function hasMovePayload(dt: DataTransfer): boolean {
        return Array.from(dt.types).some(
            (type) =>
                type === "application/aletheia-doc" ||
                type === "application/aletheia-folder",
        );
    }
    ...
    function hasDocumentPayload(dt: DataTransfer): boolean {
        return Array.from(dt.types).includes("application/aletheia-doc");
    }
    ...
    function handleDocumentVersionDrop(
        ...
        void handleDropExistingDocumentVersion(
            doc,
            e.dataTransfer.getData("application/aletheia-doc"),
        );
    }
    ...
    async function handleDropOnFolder(
        ...
        const docId = dt.getData("application/aletheia-doc");
        const subFolderId = dt.getData("application/aletheia-folder");
        if (docId) {
            const doc = (project?.documents ?? []).find((d) => d.id === docId);
