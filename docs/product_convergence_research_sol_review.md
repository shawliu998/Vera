...
- **Provider choice:** Pass. `官方法律来源 API` is available in the existing restrained native selector without changing the workbench layout. Selecting it sends the typed `official` provider; availability still fails closed when environment configuration or allowlisting is absent.
