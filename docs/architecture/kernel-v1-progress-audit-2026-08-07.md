...
| 8. Research connector | Legal and first patent adapter implemented | provider-neutral connector contract/execution; non-citable discovery versus import-only snapshot separation; CourtListener and EPO OPS Pack adapters; body-consuming central pipeline; idempotent Matter `Document`/`DocumentVersion` importer; generic provenance migration and service-only atomic commit; unit and local database/storage smokes | Bind current per-user connector authorization into Task compilation, decide whether PatSnap adds material coverage beyond EPO OPS, and run real legal/patent product gates before claiming completion. |
