...
| 8. Research connector | Legal adapter and central import implemented | provider-neutral connector contract/execution; non-citable discovery versus import-only snapshot separation; CourtListener Pack adapter; idempotent Matter `Document`/`DocumentVersion` importer; generic provenance migration and service-only atomic commit; unit and local database/storage smoke | Adapt the patent providers, bind current connector authorization into Task compilation, and run focused legal/patent research product gates before claiming completion. |
