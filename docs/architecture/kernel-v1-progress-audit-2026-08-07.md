...
| Litigation | `builtin-litigation-hearing-preparation` | evidence inventory, objection opinion, hearing outline | Pinned first-party manifest and locked synthetic litigation DOCX; pure Litigation Pack now fixes first-instance stage, represented side, output language, record-only source pins, five single-axis Evidence Inventory fields, exact DocumentVersion citation identities, stable Task-owned Review/Cell identities, and a structured per-cell reviewable-gap outcome.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Connect the Pack to one idempotent existing Tabular Review effect and bounded cell adapter; then run the fresh first-instance gold flow with direct source checks, exhausted-cell recovery, lawyer review and download.                                                   |
...
## Litigation Pack extraction slice — 2026-08-08

The preserved dirty worktree contains roughly 5,060 lines across untracked
Evidence Inventory backend, frontend and test experiments. The useful domain
ideas were inspected read-only, but the files were not copied: they combine
database writes, provider selection, prompts, retry, heuristic validation,
Task checkpoints and UI projection in one legacy application service.

The first accepted Litigation slice is instead pure Pack code under
`agent-packs/litigation`:

- one fixed first-instance hearing-preparation context requires the procedural
  stage, represented side and output language through the existing structured
  Required Input contract;
- only Matter sources typed as case-record `source` become Evidence Inventory
  rows; authority, template and precedent Versions remain distinct;
- the five fields `evidence_item`, `authenticity`, `admissibility`, `relevance`
  and `purpose_of_proof` each have a separate strict value schema and semantic
  axis instead of one overloaded status;
- every stated value and the reasoning reference typed record-evidence
  citations bound to the same fixed Document and Version, while an unknown
  value carries no fabricated quote claiming to prove absence;
- the server derives one stable Review id and one stable cell id for each fixed
  source/field pair across retries; the attempt remains provenance rather than
  Artifact identity;
- an exhausted structured-output, field-boundary or citation-relocation attempt
  yields `source_bound_generation_incomplete`, preserves all completed cells,
  returns only the current cell to `pending`, and explicitly forbids creating a
  duplicate Review.

The Pack has no route, Supabase, storage, model, user-settings or legacy-store
dependency, and the dependency boundary now enforces that rule. This slice does
not yet claim a usable Evidence Inventory: the next application slice must
publish the fixed layout through the existing Task-owned Tabular Review and an
