...
| 8. Research connector | Legal adapter in progress | provider-neutral connector contract/execution; V2 capability grants with V1 recovery; non-citable discovery versus import-only snapshot separation; CourtListener Pack adapter and focused execution tests | Add the central Matter Document/Version source importer, adapt the patent providers, bind current connector authorization, reconcile the patent import migration, and run focused research gates before claiming completion. |
